...
count_train <- all_results %>%
  ...
  mutate(
    ...
    diagnosis = recode(diagnosis, "group1" = "HCC", "group2" = "Healthy")
  )
...
count_train2$BCLC_STAGE <- ifelse(count_train2$BCLC_STAGE == "Healthy", "", count_train2$BCLC_STAGE)
...
new_df$pred <- ifelse(new_df$pred == 1, "HCC", "Healthy")
...
new_df_3 <- new_df %>% filter(pred == "HCC" & group == "test")
new_df_4 <- new_df %>% filter(pred == "Healthy" & group == "test")
new_df_5 <- new_df %>% filter(pred == "HCC" & group == "validation")
new_df_6 <- new_df %>% filter(pred == "Healthy" & group == "validation")
